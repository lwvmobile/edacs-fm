...
echo
echo "#! /bin/bash"
STRING="./rtl_udp -d ${DEVICE} -f ${FREQ} -s 28.8k -p ${PPM} -g ${GAIN} 2> /dev/null | tee >(sox -t raw -b 16 -e signed-integer -r 28800 -c 1 - -t raw - 2> /dev/null | play -t raw -r 28800 -es -b 16 -c 1 - 2> /dev/null ) | ./edacs-fm-single -${type} ${SCSV} ${GCSV} -a ${A} -f ${F} ${EXTRA} ${PATCH} ${CALL} ${VLOG} ${PLOG} ${DENY} -c ${sitevariable}" 
echo $STRING
echo
read -p "Press Enter key to start"
./rtl_udp -d $DEVICE -f 850M -s 28.8k -p $PPM -g $GAIN 2> /dev/null | tee >(sox -t raw -b 16 -e signed-integer -r 28800 -c 1 - -t raw - 2> /dev/null | play -t raw -r 28800 -es -b 16 -c 1 - 2> /dev/null ) | ./edacs-fm-single -$type $SCSV $GCSV -a $A -f $F $EXTRA $PATCH $CALL $VLOG $PLOG ${DENY} -c $sitevariable
