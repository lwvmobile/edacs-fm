...
long int BCH(long int message)
{
	int             i;
	//move some of these to run at start of edacs-fm
	//read_p();               /* Read m */
...
/* //disablign all decode and error correction parts, only generating poly and comparing to original
	...
	for (i = 0; i < k; i++) {
		data_hex = (data_hex << 1) | ddata[27-i]; //get it in the correct direction
	}

	long int recovered_hex = 0x0;
	for (i = 0; i < k; i++) {
